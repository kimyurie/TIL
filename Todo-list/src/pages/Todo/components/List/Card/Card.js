...
//ㄴ TodoList에서 보낸 속성을 매개변수로 받아올 수 있음
function TodoCard(props){
    // const {todo} = props; 
    // const {state, title, content} = todo; // ㄴ 구조분해 할당 한번 더 한다면 이렇게
    ...
    return(
        <S.Wrapper state={state}>
            ...
            <S.Content state={state}>
              {isTodoEdit ? <textarea value={editContent} onChange={onChangeEditContent}></textarea> : content}</S.Content>
        </S.Wrapper>
    );
}
